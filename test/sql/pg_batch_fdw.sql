...
CREATE TEMP TABLE pg_batch_fdw_limit_plain AS
SELECT c1, c8 FROM pg_batch_fdw_foreign WHERE c2 < 100 OFFSET 17 LIMIT 65;
SET pg_batch_limit.enable = on;
CREATE TEMP TABLE pg_batch_fdw_limit_batch AS
SELECT c1, c8 FROM pg_batch_fdw_foreign WHERE c2 < 100 OFFSET 17 LIMIT 65;
EXPLAIN (COSTS OFF)
SELECT c1, c8 FROM pg_batch_fdw_foreign WHERE c2 < 100 OFFSET 17 LIMIT 65;
SELECT NOT EXISTS (
           (TABLE pg_batch_fdw_limit_batch
            EXCEPT ALL TABLE pg_batch_fdw_limit_plain)
           UNION ALL
           (TABLE pg_batch_fdw_limit_plain
            EXCEPT ALL TABLE pg_batch_fdw_limit_batch)
       ) AS limit_fdw_matches;

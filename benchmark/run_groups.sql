...
AS $function$
DECLARE
    compressed boolean := executor_name IN
        ('arrow_batch', 'prune', 'filter', 'tam_filter');
BEGIN
    PERFORM set_config('pg_batch.enable',
                       CASE WHEN executor_name IN ('postgres', 'brin')
                            THEN 'off' ELSE 'on' END, false);
    PERFORM set_config('pg_batch.use_compressed', compressed::text, false);
...
BEGIN
    FOREACH executor_name IN ARRAY executors LOOP
        ...
        statement_name := CASE
            WHEN executor_name IN ('arrow_batch', 'prune', 'filter',
                                   'tam_filter')
                THEN 'smart_' || test_name
    ...
    FOR sample IN 1..repetitions LOOP
        SELECT array_agg(executors[((i + sample - 2) % 8) + 1] ORDER BY i)
...
ORDER BY test,
         array_position(ARRAY[
             'postgres', 'heap_batch', 'brin', 'batch_brin',
             'arrow_batch', 'prune', 'filter', 'tam_filter'], executor);
...
SELECT pg_batch_set_group_executor('tam_filter');
EXPLAIN (ANALYZE, BUFFERS OFF, COSTS OFF, TIMING OFF, SUMMARY OFF)
EXECUTE smart_range_hit;
EXPLAIN (ANALYZE, BUFFERS OFF, COSTS OFF, TIMING OFF, SUMMARY OFF)
EXECUTE smart_membership_miss;
